...
path = '<path to project>/ouput'
old_out = os.listdir(path)
for o in old_out:
    os.remove(f'output/{o}')

# change if necessary
path = '<path to project>/sources'
...
Future improvement:
-[ ] refactor as python script that takes commandline arguments to allow drag source and drop onto executable to run.
-[ ] refactor to open browser, get user to manually log in, and use tool like selenium to get html from videos in a section
    Loop:
    - click open transcript button
    - directly scrape html
